...
let () =
  Test.add_simple_test ~title:"of_array"
    (fun () ->
...
let () =
  Test.add_simple_test ~title:"reverse"
    (fun () ->
...
let () =
  Test.add_random_test ~title:"for_all" ~nb_runs:20 arr_gen
...
let () =
  Test.add_random_test ~title:"length" ~nb_runs:20 arr_gen
...
let () =
  Test.add_random_test ~title:"equal" ~nb_runs:20 arr_gen
    (fun a1 -> (R.compare (R.of_array a1) (R.of_array a1), compare a1 a1))
    [
...
let () =
  Test.add_random_test ~title:"takel" ~nb_runs:20 arr_gen
    (fun arr ->
       let range = R.of_array arr in
...
let () =
  Test.add_simple_test ~title:"drop word"
    (fun () ->
...
let () =
  Test.add_random_test ~title:"taker" ~nb_runs:20 arr_gen
    (fun arr ->
       let range = R.of_array arr in
       let len = Array.length arr in
       let take_n = if len = 0 then 0 else (Random.int len) in
